...
var Parser = module.exports = function Parser(json, config_dir) {
  this.config = json
  this.config_dir = config_dir || __dirname
  this.cwd = process.cwd()
  this.tmp_buffer = ''
  this.include_pattern = /^<!--(\s|\S)SLIDE\s@include/g
  this.comment_pattern = /<!--\s|SLIDE\s|\s-->/g
  this.first_space_pattern = /(?!\w+)\s/
}
...
Parser.prototype.render = function(buffer) {
  ...
  for (var i = 0, len = buff.length; i < len; i += 1) {
    var line = buff[i]
    this.tmp_buffer += this.analyze(line) + ((i < len - 1) ? '\n' : '')
  }
}

// Read the contents of an `@include` tag and send out for analysing
Parser.prototype.included = function(file) {
  ...
  for (var i = 0, len = buff.length; i < len; i += 1) {
    var line = buff[i]
    analyzed += this.analyze(line) + ((i < len - 1) ? '\n' : '')
  }
  return analyzed
}

// Look for the `@include` tag within a line of the buffer
Parser.prototype.analyze = function(line) {
  if (line.substring(0, 2) === '<!' && line.match(this.include_pattern)) {
...
Parser.prototype.getIncludeName = function(desc) {
  var stripped = desc.replace(this.comment_pattern, '')
  var space_index = stripped.search(this.first_space_pattern)
